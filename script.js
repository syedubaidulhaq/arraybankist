...
// Now with the help of forEach method
console.log('-------forEach-------');
...
movements.forEach(function(movement,i, array){
  if(movement > 0) {
    ...
    console.log(`Movement ${i + 1}: You widrew ${Math.abs(movement)}`);
  }
})
...
Julia and Kate are doing a study on dogs. So each of them asked 5 dog owners about their dog's age, and stored the data into an
 array (one array for each). For now, they are just interested in knowing whether a dog is an adult or a puppy.
  A dog is an adult if it is at least 3 years old, and it's a puppy if it's less than 3 years old.

Create a function 'checkDogs', which accepts 2 arrays of dog's ages ('dogsJulia' and 'dogsKate'), and does the following things:

1. Julia found out that the owners of the FIRST and the LAST TWO dogs actually have cats, not dogs! 
So create a shallow copy of Julia's array, and remove the cat ages from that copied array (because it's a bad practice to mutate 
function parameters)
...
const checkDogs = function(dogsJulia,dogsKate){
  ...
  dogs.forEach(function(dog , i){
    if(dog >= 3){
      ...
      console.log(`dog number ${i+1} is a puppy, and is ${dog} years`);
      }
  });
...
// .filter(mov => mov >= 1000).length;
  ...
  const numDeposits1000 = accounts
  .flatMap(acc => acc.movements)
  .reduce((acc, mov) => (acc >= 1000 ? ++mov : mov), 0);
  console.log(numDeposits1000);
